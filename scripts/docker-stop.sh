...
if ! docker-compose ps | grep -q "Up"; then
    ...
    orphans=$(docker ps -a --filter "label=com.docker.compose.project=arkagabriel-1" --format "{{.Names}}")
    if [[ -n "$orphans" ]]; then
        echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    # Remove ARKA images
    arka_images=$(docker images --filter "reference=*arka*" --filter "reference=*arkagabriel*" -q)
...
remaining=$(docker ps -a --filter "label=com.docker.compose.project=arkagabriel-1" --format "{{.Names}}")
if [[ -n "$remaining" ]]; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        docker ps -a --filter "label=com.docker.compose.project=arkagabriel-1" -q | xargs docker rm -f 2>/dev/null || true
